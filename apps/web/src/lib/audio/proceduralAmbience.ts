...
export class ProceduralAmbience {
  ...
  #startFire(): void {
    ...
    const pop = () => {
      ...
      const popBuf = this.#ctx.createBuffer(
        1,
        Math.floor(this.#ctx.sampleRate * popLen),
        this.#ctx.sampleRate,
      )
  ...
  stop(): void {
    ...
    try {
      this.#fireNoise?.stop()
      this.#fireNoise?.disconnect()
    } catch {}
    try {
      this.#cricketOsc1?.stop()
      this.#cricketOsc1?.disconnect()
    } catch {}
    try {
      this.#cricketOsc2?.stop()
      this.#cricketOsc2?.disconnect()
    } catch {}
  }
